...
instructions so a disassembler can follow the floating-point code.
...
every function size is unchanged.  Only the code region [0, code_end) of the load
image is touched.  This is what `../../reference/scripts/adis.py` reads; see
...
inside an instruction and the ones no instruction reached.  Needs `capstone`.
    ...
    python3 unemu87.py unf.000.exe unf.fpu.exe 0x30a00 [functions.txt]
"""
import re
import struct
import sys
...
# The segment names the decompilation uses against the paragraph each segment
# starts at in the unpacked image: `relayout.py`'s segment map, with the 1000
# Ghidra loaded the re-laid image at added to every name.
# `../../reference/scripts/adis.py` carries the same table.
PARAGRAPH = {
    0x1000: 0x0000, 0x2000: 0x057c, 0x3000: 0x1536, 0x4000: 0x24b2,
    0x5000: 0x2cb6, 0x5110: 0x2dbd, 0x5120: 0x2dcb, 0x5400: 0x3043,
}
...
def read_seeds(path):
    ...
    for line in open(path):
        ...
        if fields and ":" in fields[0]:
            ...
            seeds.append((PARAGRAPH[int(segment, 16)], int(offset, 16)))
    return seeds


def name(offset):
    paragraph = max(p for p in PARAGRAPH.values() if p * 16 <= offset)
    segment = next(s for s, p in PARAGRAPH.items() if p == paragraph)
    return "%04x:%04x" % (segment, offset - paragraph * 16)
