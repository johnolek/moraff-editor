    ...
    for m in re.finditer(b'\x2e\xff\xa7(..)', raw, re.S):
        ...
        for t in targets:
            ...
            if listing.getInstructionAt(t) is None:
                disassemble(t)
        # decompiler override
        func = fm.getFunctionContaining(jmp_addr)
        if func is not None:
            try:
                al = ArrayList()
                for t in targets: al.add(t)
                JumpTable(jmp_addr, al, True).writeOverride(func)
            except Exception as e:
                print("override failed", jmp_addr, e)
        else:
            print("no function containing", jmp_addr)
...
# rebuild all function bodies
n = 0
for f in list(fm.getFunctions(True)):
    cmd = CreateFunctionCmd(None, f.getEntryPoint(), None, SourceType.DEFAULT, False, True)
    cmd.applyTo(prog)
    n += 1
print("recreated bodies:", n)
analyzeChanges(prog)
print("FixSwitches done")
