...
| **A monster's swing lands with no beat.** `defend` stops seven times over: 1260 ms on a puffball's stat message, 110 ms before the blow line is built, 500 ms before the life drainer's box and again before a stat is drained or raised, 250 ms before POISONED / DISEASE, and 100–150 ms at the tail of every swing. The port shows the lot at once. | `defend` 2000:82b7 (unf.c 13057, 13185, 13213, 13260, 13281, 13314, 13321). | filed MORF-219 |
| **The HIT ANY KEY plaque flashes in a third of a second late.** Every eight-line box blanks the plaque's rectangle, waits 330 ms and only then scales the hand picture in and draws the words. The port draws it at once, so the rhythm every message box has is gone. | `FUN_2000_3e73` 2000:3e73, reached from `FUN_2000_4054` and its 31 callers. | filed MORF-220 |
...
| **The stone of seeing fills the map in.** It marks every square of the floor that is not rock as discovered; the port takes the stone away, says the same words and leaves the map as it was. | `use_magic_item` 2000:b202, the fourth choice: `FUN_2000_72de` over every square `solidcheck` calls open. | filed MORF-242 |
...
| **The monster manual shows the section's five monsters** across the top with the letters under them. The port draws the letters and the words. | `monster_manual` 3000:c39d, `FUN_3000_9026` 3000:9026. | filed MORF-222 |
...
| **The kill settles for a moment** between the last of its boxes and the level-0 hints about being hurt or having earned a level. | `kill_monster` unf.c 23861. | filed MORF-219 (the same "the beats the port drops" item) |
