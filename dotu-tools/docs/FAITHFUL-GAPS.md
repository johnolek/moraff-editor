...
| **The HIT ANY KEY plaque flashes in a third of a second late.** Every eight-line box blanks the plaque's rectangle, waits 330 ms and only then scales a slab of the section's wall material in, prints the two words on it and lays a frame round it whose bands crawl while it waits. | `FUN_2000_3e73` 2000:3e73, reached from `FUN_2000_4054` and its 31 callers. | fixed under MORF-220 — the crawl is the palette rotation of `FUN_4000_3b44`, which the port runs over the plaque's rectangle alone |
