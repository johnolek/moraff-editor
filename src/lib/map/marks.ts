...
export type LegendKind = { kind: 'glyph'; glyph: Glyph } | { kind: 'side'; side: Side } | { kind: 'town'; building: number } | { kind: 'open' };
...
export function squaresOfKind(rows: Square[][], floor: number, kind: LegendKind): Mark[] {
  ...
  rows.forEach((row, y) =>
    row.forEach((square, x) => {
      ...
      if (matches(square, kind)) marks.push({ x, y, label: kind.kind === 'glyph' ? String(glyphDestination(square, floor)) : null });
    }),
  );
  return marks;
}
