...
describe('squareFeature', () => {
  it('reports nothing for a plain open square', () => {
    expect(squareFeature(bundledDungeon, 0, 3, square(), 10, 10)).toBeNull();
  });

  it('sends ladders and chutes to the same square on the destination floor', () => {
    expect(squareFeature(bundledDungeon, 0, 3, square({ ladder: 2 }), 10, 11)).toEqual({
      kind: 'down',
      destination: { floor: 5, x: 10, y: 11 },
    });
    expect(squareFeature(bundledDungeon, 0, 3, square({ ladder: -1 }), 10, 11)).toEqual({
      kind: 'up',
      destination: { floor: 2, x: 10, y: 11 },
    });
    expect(squareFeature(bundledDungeon, 0, 3, square({ chute: 6 }), 10, 11)).toEqual({
      kind: 'chute',
  ...
  it('sends trap doors to the fixed landing square of the destination floor', () => {
    const [landingX, landingY] = bundledDungeon.trapdoorDest(10, 0);
    expect(squareFeature(bundledDungeon, 0, 3, square({ trapdoor: 10 }), 10, 11)).toEqual({
      kind: 'trapdoor',
  ...
  it('names town buildings', () => {
    expect(squareFeature(bundledDungeon, 0, 0, square({ town: 3 }), 10, 11)).toEqual({ kind: 'town', building: 3 });
  });
});

describe('jumpTarget', () => {
  it('only follows ladders, chutes and trap doors', () => {
    expect(jumpTarget(bundledDungeon, 0, 3, square(), 1, 1)).toBeNull();
    expect(jumpTarget(bundledDungeon, 0, 0, square({ town: 1 }), 1, 1)).toBeNull();
    expect(jumpTarget(bundledDungeon, 0, 3, square({ ladder: 1 }), 1, 1)).toEqual({ floor: 4, x: 1, y: 1 });
  });
});
