<script lang="ts">
  ...
  type Session = RollerSession | MwRollerSession | RevRollerSession;
  type View = ReturnType<RollerSession['view']> | ReturnType<MwRollerSession['view']> | ReturnType<RevRollerSession['view']>;
  ...
  function start() {
    const started =
      rolling === 'revenge' ? new RevRollerSession(slot) : rolling === 'moraffsWorld' ? new MwRollerSession(slot) : new RollerSession(slot);
    session = started;
    view = started.view();
    typed = '';
    note = '';
