...
* **`MwPlay.svelte`** — what this game puts in the Play tab: the game's screen or the top-down
  ...
  heads-up display of `../MapHud.svelte` over it — the monster's close-up, the health and spell
  orbs and the experience bar, in place of the game's own status blocks — and the panel. The tab around them is `../PlayTab.svelte`, which all three games share, and this game's
