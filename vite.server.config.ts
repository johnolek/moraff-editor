 ...
 * the run verifier, and for the same reason — the server replays runs with the engine the site is
 * built from, and a Vite build is what resolves the extensionless imports that engine is written
 * with and reads the migrations into the bundle.
 *
 ...
 * their place. `noExternal` then folds `pg` into the bundle, so what is copied to the box is
 * still one file with no `node_modules` beside it.
 *
