#!/usr/bin/env python3
"""Apply the identified names to Ghidra's raw dump of WORLD.EXE.

The Moraff's World counterpart of `dotu-tools/decomp/rename_decomp.py`, reading
its names from a module with a KNOWN dict rather than from that game's catalog
script.

Ghidra calls most functions FUN_<segment>_<offset>.  The names worked out so far
live in the KNOWN dict of ../reference/known.py, keyed by "segment:offset".
This script rewrites every FUN_ identifier whose address is in KNOWN to that
name, everywhere it appears: the "// ==== " section headers, the callers lists,
the function definitions and every call site.  Functions that were never
identified keep their FUN_ name.  Each "// ==== " header of a named function
also gets the one-line purpose appended as a trailing comment.

Usage:

    python3 rename_mw.py <raw decomp_all.c> <known.py> [output dir]
...
directory this script lives in, and the outputs are mw.c, functions.txt and
func_strings.txt.  Running the script on its own output is a no-op, so
`python3 rename_mw.py mw.c ../reference/known.py` is a way to check the
...
def read_known(known_path):
    """The KNOWN dict, read without importing the module."""
    module = ast.parse(open(known_path, encoding="utf-8").read())
    for node in module.body:
        if isinstance(node, ast.Assign) and any(
            isinstance(target, ast.Name) and target.id == "KNOWN" for target in node.targets
        ):
            return ast.literal_eval(node.value)
    raise SystemExit("no KNOWN dict in %s" % known_path)
...
def main():
    ...
    decomp_path, known_path = sys.argv[1], sys.argv[2]
    ...
    print("%d of %d named functions renamed from FUN_; wrote %s to %s"
