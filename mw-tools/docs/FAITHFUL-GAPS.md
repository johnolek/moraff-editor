...
|---|---|---|
| **The game makes four noises and the port makes none.** A rising chirp when your blow lands, a falling one when a monster hits you, a two-note chime when a monster dies, a 1.4-second dirge when you die — the same four cues, at the same frequencies, as Dungeons of the Unforgiven. The chime and the dirge both start *before* their words appear. | `FUN_2000_5bb6` (2000:5bb6) from `strike`, `FUN_2000_6123` (2000:6123) from `monster_turn`, `FUN_3000_d4d9` (3000:d4d9) first in `monster_killed`, `FUN_2000_722c` (2000:722c) first in the death routine. All gate on DS:119f, the O key's own flag. | filed MORF-227 |
| **The level, hit points and kill value of every monster beside you** stand over all four views at once, not just the one you face. Being surrounded is ordinary play and three of the four readouts are missing. | `movecontrol` calls `FUN_2000_8728` (2000:8728) per occupied side; `FUN_2000_8b3f` (2000:8b3f) calls `FUN_2000_892d` (2000:892d) per side. `drawMonsterInfo` in the port is a faithful copy with no caller. | filed MORF-228 |
| **A fight reads out over about a second.** A swing blanks its strip for a tenth of a second first, so swinging twice with the same result still blinks; each of a monster's attacks clears, waits 110 ms, prints, and holds 100 ms on a miss or 350 ms on a hit; a puffball's line hangs 1260 ms. Death holds the killing message 1300 ms first, and a jammed door holds 350 ms. | `strike` 2000:5bef, `monster_turn` 2000:615c through `FUN_2000_7fb1` (2000:7fb1), `movecontrol` 2000:aad5, `monster_killed` 3000:d51c. | filed MORF-229 |
...
| **The little mouse's advice is colour-coded** — a different colour for each of the eight pieces — **and stands in the bottom half of the box**, rows 5 to 8, not the top. | `FUN_3000_9383` 3000:9383 and `FUN_3000_8b27` 3000:8b27. The port's own comment in `advice.ts` says the lines go down the right-hand panel; x is 0, so that is wrong too. | filed MORF-230 |
| **The expanded map points at the level's quest boss** — GO NORTH / SOUTH / EAST / WEST in red — and blinks the character's square through the whole palette rather than through sixteen colours. | `FUN_2000_a8d7` 2000:a8d7 from the X branch; the wider blink is `FUN_2000_7d00` 2000:7d00. | filed MORF-231 |
| **The map cursor blinks** through sixteen colours as fast as the machine runs. | `FUN_2000_7c8a` 2000:7c8a. | filed MORF-221's sibling problem — the port draws it in one colour and says so in `MwScreen.svelte`; not worth a second item until DotU's arrow flash has an answer |
| The town buildings' menus have no picture behind them. | `store`, `inn`, `bank`, `FUN_2000_3085`. | not worth it — **the README is wrong about this**: those four draw no picture at all. The ceiling mark above is the town's only art, and it is fixed |
